...
abstract class MoveLogin_Settings extends MoveLogin_Singleton {
	...
	protected function add_section( $title, $args = null ) {
		...
		if ( ! empty( $args['with_roles'] ) ) {
			$actions .= '<button type="button" id="affected-role-' . $i . '" class="hide-if-no-js no-button button-actions-title">' . __( 'Roles', 'movelogin' ) . ' <span class="dashicons dashicons-arrow-right" aria-hidden="true"></span></button>';
		}
		...
		$this->add_field( array(
			'title'        => '<span class="dashicons dashicons-groups"></span> ' . __( 'Affected Roles', 'movelogin' ),
			'description'  => __( 'Which roles does this module affect?', 'movelogin' ),
			'depends'      => 'affected-role-' . $i,
			'row_class'    => 'affected-role-row',
			'name'         => $this->get_field_name( 'affected_role' ),
			'type'         => 'roles',
			'label_screen' => __( 'Affected Roles', 'movelogin' ),
			'helpers'      => array(
				array(
					'type'        => 'description',
					'description' => __( 'Future roles will be automatically checked.', 'movelogin' ),
				),
				array(
					'type'        => 'warning',
					'class'       => 'hide-if-js',
					'description' => __( 'Select 1 role minimum', 'movelogin' ),
				),
			),
		) );
	...
	protected function do_sections() {
		 ...
		 */
		do_action( 'movelogin.settings.before_section_' . $this->sectionnow, $with_save_button );

		$this->section_is_disabled = true;

		echo '<div class="movelogin-settings-section" id="movelogin-settings-' . $html_id . '">';
		...
		if ( $with_save_button ) {
			 ...
			 */
			$args = apply_filters( 'movelogin.settings.section.submit_button_args', $args, $this->sectionnow );
			/**
			 * Filter the arguments passed to the section submit button.
			 *
			 * @since 1.4.3
			 ...
			 * @param (array) $args An array of arguments passed to the `submit_button()` method.
			 */
			$args = apply_filters( 'movelogin.settings.section-' . $this->sectionnow . '.submit_button_args', $args );

			call_user_func( array( __CLASS__, 'submit_button' ), $args );
		}

		echo '</div><!-- #movelogin-settings-' . $html_id . ' -->';
		 ...
		 * @param (bool) $with_save_button True if a "Save All Changes" button will be printed.
		 */
		do_action( 'movelogin.settings.after_section_' . $this->sectionnow, $with_save_button );
	...
	final protected function do_settings_sections() {
		...
		foreach ( (array) $wp_settings_sections[ $section_id ] as $section ) {
			...
			if ( $section['title'] ) {
				echo '<div class="movelogin-settings-section-header">';
				$header_open_tag = true;
				$id = explode( '|', $section['id'] );
				$id = end( $id );
				echo '<h3 class="movelogin-settings-section-title" id="module-' . sanitize_html_class( $id ) . '">' . $section['title'] . '</h3>' . "\n";
			}

			if ( $section['callback'] ) {
				echo ( $header_open_tag ? '' : '<div class="movelogin-settings-section-header">' );
				$header_open_tag = true;
				call_user_func( $section['callback'], $section );
			}

			echo ( $header_open_tag ? '</div><!-- .movelogin-settings-section-header -->' : '' );

			if ( ! isset( $wp_settings_fields ) || ! isset( $wp_settings_fields[ $section_id ] ) || ! isset( $wp_settings_fields[ $section_id ][ $section['id'] ] ) ) {
				continue;
			}

			echo '<div class="movelogin-form-table">';
	...
	protected function field( $args ) {
		...
		if ( $args['plugin_activation'] ) {
			$option_name = 'movelogin-plugin-activation';
		} else {
			$option_name = 'movelogin' . ( 'global' !== $this->modulenow ? '_' . $this->modulenow : '' ) . '_settings';
		}
		...
		if ( isset( $args['value'] ) ) {
			...
			$value = movelogin_get_option( $args['name'] );
		} else {
			$value = movelogin_get_module_option( $args['name'] );
		}
		...
		if ( 'radioboxes' === $args['type'] || 'checkboxes' === $args['type'] || 'checkbox' === $args['type'] || 'roles' === $args['type'] ) {
			$args['attributes']['class'][] = 'movelogin-checkbox';
		}
		if ( 'countries' === $args['type'] ) {
			$args['attributes']['class'][] = 'movelogin-checkbox';
			$args['attributes']['class'][] = 'movelogin-checkbox-mini';
		}

		if ( 'radios' === $args['type'] || 'roles_radio' === $args['type'] ) {
			$args['attributes']['class'][] = 'movelogin-radio';
		}
		...
		if ( $has_fieldset_begin ) {
			$toggle_all = isset( $args['toggle_all'] ) && $args['toggle_all'] ? 'movelogin-check-group' : '';
		...
		if ( '' !== $args['label_before'] || '' !== $args['label'] || '' !== $args['label_after'] ) {
			$label_open  = '<label class="movelogin-' . esc_attr( $args['type'] ) . '-label' . ( $disabled ? ' disabled' : '' ) . '">';
			$label_close = '</label>';
		}
		...
		switch ( $args['type'] ) {
			case 'url' :
				if ( movelogin_is_pro() ) {
					add_action( 'admin_footer', 'movelogin_pro_enqueue_wplink_dialog' );
				}
			case 'number' :
			case 'email' :
			case 'tel' :
			...
			case 'textarea' :
				...
				$args['label_before'] .= $is_exception ? '' : '<div class="movelogin-textarea-container">';
				$args['label_after'] .= $is_exception ? '' : '</div>';

				echo $label_open; ?>
					<?php
					echo $args['label'] ? '<span class="movelogin-bold">' . $args['label'] . '</span><br/>' : '';
					echo $args['label_before'];
				...
				echo $label_open; ?>
					...
					<select id="<?php echo $args['label_for']; ?>" name="<?php echo $name_attribute; ?>"<?php echo $attributes; ?>>
						...
						foreach ( $args['options'] as $val => $title ) {
							$disabled = '';
							if ( static::is_pro_feature( $args['name'] . '|' . $val ) && ! movelogin_is_pro() ) {
			...
			case 'checkbox' :
				...
				echo '<p class="movelogin-checkbox-line">';
				echo $label_open; ?>
					<?php
					echo $args['label_before'];
			...
			case 'radioboxes' :
				...
				$_all_css   = $attributes;
				// if ( $toggle_all ) {
				// 	$args['options'] = array_merge( [ 0 => __( 'Toggle All', 'movelogin' ) ], $args['options'] );
				// 	$_all_css = str_replace( 'class="', 'class="movelogin-toggle-check ', $attributes );
				// }
				foreach ( $args['options'] as $val => $title ) {
					if ( ! is_array( $title ) ) {
						$title = (array) $title;
					...
					foreach ( $title as $_title ) {
						...
						$disabled          = ( static::is_pro_feature( $args['name'] . '|' . $_value ) && ! movelogin_is_pro() ) ? ' disabled="disabled"' : $disabled;
						...
						if ( static::is_pro_feature( $args['name'] . '|' . $_value ) && ! movelogin_is_pro() ) {
							$classes      .= ' movelogin-pro-option';
						} elseif ( static::is_pro_feature( $args['name'] . '|' . $_value ) ) {
							$classes      .= ' movelogin-show-pro';
						}
						if ( static::is_expert_feature( $args['name'] . '|' . $_value ) ) {
							$classes      .= ' movelogin-show-expert';
						}
						// $toggle_all = false;
						// class="movelogin-row-check
						?>
						<p class="movelogin-fieldset-item movelogin-fieldset-item-radio movelogin-fieldset-item-<?php echo $args['type']; ?> movelogin-field-<?php echo esc_attr( $args['label_for'] ); ?><?php echo $classes; ?>">
						...
						<?php echo static::is_pro_feature( $args['name'] . '|' . $_value ) && ! movelogin_is_pro() ? static::get_pro_version_string( '<span class="description movelogin-get-pro-version">%s</span>' ) : ''; ?>
						</p>
						<?php
						$_all_css = $attributes;
					}
			...
			case 'radios' :
				...
				foreach ( $args['options'] as $val => $title ) {
					...
					$disabled          = static::is_pro_feature( $args['name'] . '|' . $val ) && ! movelogin_is_pro() ? ' disabled="disabled"' : '';
					$pro_class         = '';
					if ( static::is_pro_feature( $args['name'] . '|' . $val ) && ! movelogin_is_pro() ) {
						$pro_class     = ' movelogin-pro-option';
					} elseif ( static::is_pro_feature( $args['name'] . '|' . $val ) ) {
						$pro_class     = ' movelogin-show-pro';
					}
					if ( static::is_expert_feature( $args['name'] . '|' . $val ) ) {
						$pro_class    .= ' movelogin-show-expert';
					}

					if ( ! $disabled && strpos( $title, 'movelogin-coming-soon-feature' ) !== false ) {
						$disabled      = ' disabled="disabled"';
					}
					?>
					<p class="movelogin-radio-line<?php echo $pro_class; ?>">
						<label<?php echo $disabled ? ' class="disabled"' : ''; ?> for="<?php echo esc_attr( $args['label_for'] ); ?>">
						...
						<?php echo static::is_pro_feature( $args['name'] . '|' . $val ) && ! movelogin_is_pro() ? static::get_pro_version_string( '<span class="description movelogin-get-pro-version">%s</span>' ) : ''; ?>
					</p>
					<?php
				}
				break;
			...
			case 'roles_radio' :
				...
				$roles = array_map( 'movelogin_translate_user_role', $roles );
			...
			case 'roles' :
				...
				foreach ( $roles as $val => $title ) {
					?>
					<p class="movelogin-checkbox-roles-line">
			...
			case 'submit' :
				...
				echo '<button type="submit" class="movelogin-button" id="' . esc_attr( $args['name'] ) . '">' . $args['label'] . '</button>';
				break;
			...
			case 'plugin' :
				 ...
				 */
				// Look in the cache.
				$plugin_info = get_transient( "movelogin_plugin_cards_{$args['name']}" );
				...
				if ( ! $plugin_info ) {
					...
					if ( is_object( $plugin_info ) && ! is_wp_error( $plugin_info ) ) {
						set_transient( "movelogin_plugin_cards_{$args['name']}", $plugin_info, $expiration );
					}
				}
				// Default $output.
				$output = '';
				...
				if ( is_object( $plugin_info ) && ! is_wp_error( $plugin_info ) ) {
					...
					$output .= movelogin_render_plugin_card( $plugin_info );
					$output .= '</div>';
				}
				echo $output;
			...
			default :
				if ( movelogin_is_pro() && function_exists( 'movelogin_pro_' . $args['type'] . '_field' ) ) {
					call_user_func( 'movelogin_pro_' . $args['type'] . '_field', $args, $this );
	...
	protected function reset_settings_button() {
		?>
		<a class="movelogin-button button button-secondary" href="<?php echo wp_nonce_url( admin_url( 'admin-post.php?action=movelogin_reset_all_settings' ), 'movelogin_reset_all_settings' ); ?>">
			<?php _e( 'Reset Settings', 'movelogin' ); ?>
		</a>
		<?php
	}
	...
	protected function tables_selection() {
		...
		$non_wp_tables = movelogin_get_non_wp_tables();
		$wp_tables     = movelogin_get_wp_tables();
		$blog_ids      = ! is_multisite() ? [ '1' ] : $wpdb->get_col( "SELECT blog_id FROM {$wpdb->blogs}" );

		$form  = '<div class="show-input">';
		$form .= '<fieldset aria-labelledby="select-db-tables-to-rename" class="movelogin-boxed-group">';

		if ( $non_wp_tables ) {
			$form .= '<b>' . __( 'Other tables', 'movelogin' ) . '</b><br/>';
			foreach ( $non_wp_tables as $table ) {
				$table_attr = esc_attr( $table );
				$form      .= '<input type="checkbox" name="movelogin_wordpress-core_settings[database_tables_selection][]" value="' . $table_attr . '" id="select-db-tables-to-rename-' . $table_attr . '" checked="checked"><label for="select-db-tables-to-rename-' . $table_attr . '">' . esc_html( $table ) . '</label><br/>';
			}
		}

		$form .= '<b>' . __( 'WordPress tables (mandatory)', 'movelogin' ) . '</b><br/>';
	...
	protected function import_upload_form() {
		...
		$disabled   = movelogin_is_pro() ? '' : ' disabled="disabled"';

		if ( ! empty( $upload_dir['error'] ) ) {
			?>
			<div class="error">
				<p><?php _e( 'Before you can upload your import file, you will need to fix the following error:', 'movelogin' ); ?></p>
				<p><strong><?php echo $upload_dir['error']; ?></strong></p>
			</div><?php
			echo movelogin_is_pro() ? '' : static::get_pro_version_string( '<p class="description movelogin-get-pro-version">%s</p>' );
			return;
		}

		$name        = 'upload';
		$type        = 'help';
		$description = __( 'Choose a file from your computer:', 'movelogin' ) . ' (' . sprintf( __( 'Maximum size: %s', 'movelogin' ), $size ) . ')';
		/** This filter is documented in inc/classes/settings/class-movelogin-settings.php */
		$description = apply_filters( 'movelogin.settings.help', $description, $name, $type );
		?>
		<p>
		...
		<p class="submit">
			<button type="submit"<?php echo $disabled; ?> class="movelogin-button" id="import">
				<span class="icon">
					<i class="movelogin-icon-upload" aria-hidden="true"></i>
				</span>
				<span class="text">
					<?php _e( 'Upload Settings', 'movelogin' ); ?>
				</span>
			</button>
		</p>
		<?php
	...
	protected function export_form() {
		...
		<p class="submit">
			<?php if ( movelogin_is_pro() ) : ?>
				<a href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=movelogin_export' ), 'movelogin_export' ) ); ?>" id="export" class="movelogin-button">
					<span class="icon" aria-hidden="true">
						<i class="movelogin-icon-download"></i>
					</span>
					<span class="text">
						<?php _e( 'Download settings', 'movelogin' ); ?>
					</span>
				</a>
			<?php else : ?>
				<button type="button" class="movelogin-button" disabled="disabled">
					<span class="icon" aria-hidden="true">
						<i class="movelogin-icon-download"></i>
					</span>
					<span class="text">
	...
	protected function field_button( $args ) {
		...
		if ( ! empty( $args['label'] ) ) {
			$class    = 'movelogin-button moveloginicon-' . sanitize_html_class( $args['name'] );
			$class   .= ! empty( $args['style'] )    ? ' movelogin-button-' . $args['style'] : '';
	...
	protected static function helpers( $args ) {
		...
		foreach ( $args['helpers'] as $helper ) {
			...
			switch ( $type ) {
				...
				break;
				case 'warning' :
					$desc  = '<' . $tag . ' class="description warning' . $depends . $class . '">' . ( 'p' === $tag ? '' : '<p>' ) . '<strong>' . __( 'Warning: ', 'movelogin' ) . '</strong> ' . $helper['description'] . '</' . $tag . '>'; // Don't forget to close the <p> tag.
				break;
				default :
					continue 2;
			}
			 ...
			 */
			echo apply_filters( 'movelogin.settings.help', $desc, $name, $type );
			/**
			 ...
			 * @param (string) $type The helper type.
			 */
			echo apply_filters( 'movelogin.settings.help.forced', $force, $name, $type );
		}
	}
	...
	protected function add_field( $args ) {
		...
		if ( false !== apply_filters( 'movelogin.settings.field.' . $args['name'], false ) ) {
			return;
		}
		...
		if ( is_array( $args['field_type'] ) ) {
			...
			$callback = 'movelogin_' . $args['field_type'];
		}
		 ...
		 * @since 1.0
		 */
		do_action( 'movelogin.settings.after_field_' . $this->modulenow . '|' . $this->pluginnow );
	...
	final protected function do_settings_fields( $page, $section ) {
		...
		foreach ( (array) $wp_settings_fields[ $page ][ $section ] as $field ) {
			...
			$is_free  = ! movelogin_is_pro();
			$data_f   = '';
			$class    = 'movelogin-setting-row_' . sanitize_html_class( $field_id ) . ' movelogin-setting-row ';
			$class   .= static::is_pro_feature( $field['args']['name'] ) && $is_free ? 'movelogin-pro-row ' : '';
			$class   .= static::is_pro_feature( $field['args']['name'] ) && ! $is_free ? 'movelogin-show-pro ' : '';
			$class   .= static::is_expert_feature( $field['args']['name'] ) ? 'movelogin-show-expert ' : '';
			...
			<div<?php echo $id . $class . $data_f; ?>>
				<div class="movelogin-flex">
					<div class="movelogin-setting-content-col">
					<?php
			} else {
				echo '<div class="movelogin-setting-row-hidden">';
			}
					// Row title.
					if ( $field['title'] ) {
						...
						if ( ! empty( $field['args']['label_for'] ) ) {
							...
							echo '<label for="' . esc_attr( $field['args']['label_for'] ) . '" class="movelogin-setting-row-title">' . $field['title'] . '</label>';
						} else {
							echo '<h4 id="row-' . sanitize_html_class( $field_id ) . '" class="movelogin-setting-row-title">' . $field['title'] . '</h4>';
						}
					}
					...
					*/
					if ( $field['args']['description'] && apply_filters( 'movelogin.settings.description', true ) ) {
					...
					<div class="movelogin-get-pro-col">
					<?php
					}
					if ( static::is_pro_feature( $field['args']['name'] ) && $is_free ) {
						echo '<p class="movelogin-get-pro">' . static::get_pro_version_string() . '</p>';
					}
					if ( 'hidden' !== $field['args']['type'] ) {
					?>
					</div><!-- .movelogin-get-pro-col -->
				</div><!-- .movelogin-flex -->
			</div>
			<?php
					} else {
	...
	protected function field_is_disabled( $field_args ) {
		...
		if ( ! empty( $field_args['disabled'] ) ) {
			...
			$disabled = true;
		} elseif ( static::is_pro_feature( $field_args['name'] ) && ! movelogin_is_pro() ) {
			// The field is Pro.
			$disabled = true;
		} elseif ( ! empty( $field_args['options'] ) && ! movelogin_is_pro() ) {
			...
			foreach ( (array) $field_args['options'] as $val => $title ) {
				...
				if ( ! static::is_pro_feature( $field_args['name'] . '|' . $val ) && ! movelogin_is_pro() ) {
					$has_enabled = true;
					$fields[ $name ] = false;
				} else {
					$fields[ $name ] = true;
	...
	protected function print_section_description() {
		...
		if ( ! empty( $this->section_descriptions[ $key ] ) && apply_filters( 'movelogin.settings.description', true ) ) {
			echo '<div class="movelogin-settings-section-description">';
	...
	protected static function submit_button( $args ) {
		$defaults = [ 'label' => __( 'Save All Changes', 'movelogin' ), 'before' => '', 'type' => 'primary large', 'name' => 'main_submit', 'wrap' => true, 'other_attributes' => null, 'echo' => true ];
		...
		$classes          = array( 'movelogin-button' );

		foreach ( $type as $t ) {
			$classes[] = isset( $button_shorthand[ $t ] ) ? 'movelogin-button-' . $t : $t;
		}
	 ...
	 * @since 1.2   A method of this class. Was previously `movelogin_print_sideads()`.
	 * @author Geoffrey Crofte
	 */
	protected function print_sideads() {
		global $current_screen;

		if ( 'settings_page_movelogin_modules' !== $current_screen->base ) {
			return;
		}
		/**
		 ...
		 */
		if ( false !== apply_filters( 'movelogin.no_sidebar', false ) ) {
			return;
		}

		?>
		<div class="movelogin-sideads">
		<?php
		/**
		 ...
		 * @param (bool) false by default
		 */
		if ( false === apply_filters( 'movelogin.no_sideads', false ) ) { // Filter movelogin_no_sideads.
			...
			// 	$sideads = [
			// 				0 => [
			// 					'hook'    => 'movelogin_ad_before',
			// 					'when'    => 'free',
			// 					'content' => '<div class="movelogin-section-dark movelogin-pro-ad"> <i class="icon-movelogin" aria-hidden="true"></i> <img src="https://secupress.me/wp-content/uploads/edd/blackfriday@2x.png" class="movelogin-pro-icon" srcset="https://secupress.me/wp-content/uploads/edd/blackfriday@2x.png" width="80" height="64" alt="SecuPress Pro"/> <p class="movelogin-text-medium">Black Friday Offer</p> <p>Unlock all the features of <strong>SecuPress Pro</strong></p> <a href="https://secupress.me/pricing/" class="movelogin-button movelogin-button-tertiary movelogin-button-getpro"> <span class="text">Lifetime Limited Edition</span> </a> <p><a href="https://secupress.me/features">or Learn More About Pro Features</a></p> </div>',
			// 					'content-fr_FR' => '<div class="movelogin-section-dark movelogin-pro-ad"> <i class="icon-movelogin" aria-hidden="true"></i> <img src="https://secupress.me/wp-content/uploads/edd/blackfriday@2x.png" class="movelogin-pro-icon" srcset="https://secupress.me/wp-content/uploads/edd/blackfriday@2x.png" width="80" height="64" alt="SecuPress Pro"/> <p class="movelogin-text-medium">Offre du Black Friday</p> <p>Débloquez toutes les fonctionnalités<br>de <Strong>SecuPress Pro</strong></p> <a href="https://secupress.me/fr/tarifs/?discount=BF2019" class="movelogin-button movelogin-button-tertiary movelogin-button-getpro"> <span class="text">Edition Limitée "Lifetime"</span> </a> <p><a href="https://secupress.me/fr/fonctionnalites/">ou découvrez les fonctionnalités pro</a></p> </div>',
			// 					],
			// 				];
			// } else {
				$sideads = [
							0 => [
								...
								'when'    => 'free',
								'content' => 
									sprintf( '<div class="movelogin-section-dark movelogin-pro-ad"><i class="icon-movelogin" aria-hidden="true"></i><img src="##MOVELOGIN_ADMIN_IMAGES_URL##logo-pro.png" class="movelogin-pro-icon" srcset="##MOVELOGIN_ADMIN_IMAGES_URL##logo-pro@2x.png" width="80" height="78" alt="SecuPress Pro"/><p class="movelogin-text-medium">%s</p><p>%s</p><a href="%s" class="movelogin-button movelogin-button-tertiary movelogin-button-getpro"><span class="text">%s</span></a><p><a href="%s">%s</a></p></div>',
										__( 'Improve your Security', 'movelogin' ),
										__( 'Unlock all the features of SecuPress Pro', 'movelogin' ),
										trailingslashit( set_url_scheme( MOVELOGIN_WEB_MAIN, 'https' ) ) . _x( 'pricing', 'link to website (Only FR or EN!)', 'movelogin' ),
										_x( 'Get SecuPress Pro', 'ads', 'movelogin' ),
										trailingslashit( set_url_scheme( MOVELOGIN_WEB_MAIN, 'https' ) ) . _x( 'features', 'link to website (Only FR or EN!)', 'movelogin' ),
										__( 'Learn More About Pro Features', 'movelogin' ),
									),
								//'content' => '<div class="movelogin-section-dark movelogin-pro-ad"> <i class="icon-movelogin" aria-hidden="true"></i> <img src="##MOVELOGIN_ADMIN_IMAGES_URL##logo-pro.png" class="movelogin-pro-icon" srcset="##MOVELOGIN_ADMIN_IMAGES_URL##logo-pro@2x.png" width="80" height="78" alt="SecuPress Pro"/> <p class="movelogin-text-medium">Améliorez votre sécurité</p> <p>Débloquez toutes les fonctionnalités<br>de SecuPress Pro</p> <a href="https://secupress.me/fr/tarifs/" class="movelogin-button movelogin-button-tertiary movelogin-button-getpro"> <span class="text">Acheter la version Pro</span> </a> <p><a href="https://secupress.me/fr/fonctionnalites/">Découvrez les fonctionalités pro</a></p> </div>',
								],
							];
			// }

			foreach ( $sideads as $sidead ) {
				if ( 'movelogin_ad_before' !== $sidead['hook'] ) {
					continue;
				}
				if ( ( 'free' === $sidead['when'] && ! movelogin_has_pro() )
					|| ( 'pro' === $sidead['when'] && movelogin_has_pro() )
					|| 'both' === $sidead['when']
				 ) {
				 	$content_locale = 'content-' . get_user_locale();
				 	$content        = isset( $sidead[ $content_locale ] ) ? $sidead[ $content_locale ] : $sidead['content'];
					echo wp_kses_post( str_replace( '##MOVELOGIN_ADMIN_IMAGES_URL##', MOVELOGIN_ADMIN_IMAGES_URL, $content ) );
				}
			}
			 ...
			 * @since 1.4
			 */
			do_action( 'movelogin.ad_before' );

		}
		?>

		<div class="movelogin-bordered movelogin-mail-ad">
			<div class="movelogin-ad-header movelogin-flex">
				<span><i class="dashicons dashicons-plus-alt movelogin-primary" aria-hidden="true"></i></span>
				<p><?php _e( 'Hacked Website?', 'movelogin' ); ?></p>
			</div>
			<div class="movelogin-ad-content-padded movelogin-ad-content">
				<p><?php _e( 'Well, this is not a good day for you, we will try to make you smile while we\'re working on it!', 'movelogin' ) ?></p>

			<p class="movelogin-cta">
				<?php
				$hacked_url = movelogin_admin_url( 'get-pro' ) . '#services';
				$hacked_target = '_blank';
				?>
				<a href="<?php echo esc_url( $hacked_url ); ?>" class="movelogin-button" target="<?php echo esc_attr( $hacked_target ); ?>"><?php _e( 'Ask an Expert', 'movelogin' ); ?></a>
			</p>
			</div>
		</div>
	 ...
	 *
	 * @since 1.4 Do not check anymore if movelogin_is_pro()
	...
	public static function is_pro_feature( $value ) {
		return movelogin_feature_is_pro( $value );
	}
	...
	public static function is_expert_feature( $value ) {
		return movelogin_feature_is_pro( $value ) && ! movelogin_is_pro() ? false : movelogin_feature_is_expert( $value );
	}
	...
	public static function get_pro_version_string( $format = '' ) {
		...
		if ( current_user_can( 'install_plugins' ) ) {
			$install_url = admin_url( 'plugin-install.php?tab=search&s=secupress&type=term' );
			$message = sprintf(
				__( 'Available in the <a href="%s">full SecuPress version</a>', 'movelogin' ),
				esc_url( $install_url )
			);
		} else {
			$message = __( 'Available in the full SecuPress version', 'movelogin' );
		}

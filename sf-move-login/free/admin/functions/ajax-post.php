 ...
 */
function movelogin_admin_die( $data = null ) {
	if ( wp_doing_ajax() ) {
	...
	if ( is_wp_error( $data ) ) {
		...
		if ( wp_get_referer() ) {
			...
			$data .= sprintf( '<a href="%s">%s</a>',
				...
				__( 'Please try again.', 'movelogin' )
			);
		}
	}

	wp_die( $data, __( 'WordPress Failure Notice', 'movelogin' ), 403 );
}
 ...
 */
function movelogin_admin_send_response_or_redirect( $response = false, $redirect = false ) {
	if ( ! $response ) {
		movelogin_admin_die( 'Missing $response in ' . __FUNCTION__ ); // Do not translate.
	}
	...
	if ( $redirect ) {
		...
		$redirect = movelogin_admin_url( $redirect[0] ) . ( ! empty( $redirect[1] ) ? '#' . $redirect[1] : '' );
	} else {
 ...
 *                      (string)      $code        An error code used by `movelogin_add_settings_error()`.
 *                      (string)      $type        `success` (default) or `error`. Will decide to send a success or an error message.
 **/
function movelogin_admin_send_message_die( $args ) {
	...
	if ( ! $args['redirect_to'] ) {
		...
		$args['redirect_to'] = movelogin_admin_url( $args['redirect_to'] );
	}

	$args['type'] = 'success' === $args['type'] ? 'updated' : 'error';

	movelogin_add_settings_error( 'general', $args['code'], $args['message'], $args['type'] );
	set_transient( 'settings_errors', movelogin_get_settings_errors(), 30 );
 ...
 */
function movelogin_check_user_capability( $force_mono = false ) {
	if ( ! current_user_can( movelogin_get_capability( $force_mono ) ) ) {
		movelogin_admin_die();
	}
}
 ...
 */
function movelogin_check_admin_referer( $action = -1, $query_arg = '_wpnonce' ) {
	if ( wp_doing_ajax() ) {

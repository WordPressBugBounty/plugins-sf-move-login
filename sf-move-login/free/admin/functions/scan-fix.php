 ...
 */
function movelogin_scanit( $test_name, $format_response = false, $for_current_site = false ) {
	$response          = false;
	...
	if ( ! file_exists( movelogin_class_path( 'scan', $test_name ) ) ) {
	...
	movelogin_require_class( 'scan' );
	movelogin_require_class( 'scan', $test_name );

	$classname = 'SecuPress_Scan_' . $test_name;

	if ( class_exists( $classname ) ) {
		ob_start();
		movelogin_time_limit( 0 );
	...
	if ( $response ) {
		$formated_response = array(
			'status'  => movelogin_status( $response['status'] ),
			'class'   => sanitize_key( $response['status'] ),
			'message' => isset( $response['msgs'] )    ? movelogin_format_message( $response['msgs'], $test_name )    : '',
			'fix_msg' => isset( $response['fix_msg'] ) ? movelogin_format_message( $response['fix_msg'], $test_name ) : '',
		);
	}
	/**
	...
	*/
	do_action( 'movelogin.scanit.response', $test_name, $formated_response );
 ...
 */
function movelogin_scanit_async( $test_name, $delay = 0 ) {
	$http_args = [
		'timeout'   => 0.01,
		'blocking'  => false,
		'cookies'   => $_COOKIE,
	...
	$nonce_action   = 'movelogin_scanner_' . $test_name . ( $is_subsite ? '-' . $site_id : '' );
	$delay          = max( min( 5, (int) $delay ), 0 );
	$delay          = $delay ? '&delay=' . $delay : '';
	$scanner_url    = admin_url( 'admin-ajax.php?action=movelogin_scanner' . $delay . '&test=' . $test_name . ( $is_subsite ? '&for-current-site=1&site=' . $site_id : '' ) );
 ...
 * @return (array|bool) The scan result or false on failure.
 */
function movelogin_fixit( $test_name, $format_response = false, $for_current_site = false ) {
	$response = false;

	if ( ! $test_name || ! file_exists( movelogin_class_path( 'scan', $test_name ) ) ) {
		return false;
	}
 ...
 * @return (array|bool) The scan result or false on failure.
 */
function movelogin_manual_fixit( $test_name, $format_response = false, $for_current_site = false ) {
	$response = false;
...
add_action( 'admin_footer', 'movelogin_pre_check_php_version' );
/**
 ...
 **/
function movelogin_pre_check_php_version() {
	$info     = get_option( 'movelogin_scan_phpversion' );
	if ( ! isset( $info['status'] ) || 'bad' !== $info['status'] ) {
		return;
	}
	$versions = movelogin_get_php_versions();
	if ( version_compare( $versions['current'], $versions['mini'] ) >= 0 ) {
		movelogin_scanit( 'PhpVersion' );
	}
}

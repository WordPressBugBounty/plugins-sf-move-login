 ...
 */
function movelogin_get_submodule_activations( $module ) {
	static $done = [];
	...
	if ( isset( $_POST['option_page'] ) && 'movelogin_' . $module . '_settings' === $_POST['option_page'] ) { // WPCS: CSRF ok.
		$return = isset( $_POST['movelogin-plugin-activation'] ) && is_array( $_POST['movelogin-plugin-activation'] ) ? $_POST['movelogin-plugin-activation'] : array(); // WPCS: CSRF ok.
		$done[ $module ] = $return;
		return $return;
	}
 ...
 */
function movelogin_manage_affected_roles( &$settings, $module, $plugin ) {
	static $roles;
	...
	if ( empty( $settings[ $plugin . '_affected_role' ]['witness'] ) ) {
		...
		$old_settings = get_site_option( "movelogin_{$module}_settings" );
 ...
 */
function movelogin_get_deactivate_plugin_string( $plugin_basename ) {
	if ( ! movelogin_is_plugin_active( $plugin_basename ) ) {
		return null;
	}
	...
	return sprintf( __( 'will deactivate the plugin %s.', 'movelogin' ), '<strong>' . $plugin['Name'] . '</strong>' );
}
 ...
 * @return (string|null) Return null if the plugin is not activated.
 */
function movelogin_plugin_in_usage_string( $plugin_basename, $settings_page = '' ) {
	if ( ! movelogin_is_plugin_active( $plugin_basename ) ) {
		return null;
	}
	...
	$content = sprintf( __( 'You cannot use this feature now because you are using the plugin %s. Please deactivate it.', 'movelogin' ), '<strong>' . esc_html( $plugin['Name'] ) . '</strong>' );
	if ( $settings_page ) {
		$content .= sprintf( '<br><a href="%s">' . __( 'Open the %s settings page', 'movelogin' ) . '.</a>', esc_url( admin_url( $settings_page ) ), '<strong>' . esc_html( $plugin['Name'] ) . '</strong>' );
	}

 ...
 */
function movelogin_get_option( $option, $default = false ) {
	$pre = null;
	/**
	 ...
	 */
	$value = apply_filters( 'pre_movelogin_get_option_' . $option, $pre, $default );
	 ...
	 * @param (mixed) $default The default value.
	 */
	return apply_filters( 'movelogin_get_option_' . $option, $value, $default );
}
 ...
 */
function movelogin_set_option( $option, $value ) {
	/**
	 ...
	 */
	$value = apply_filters( 'pre_movelogin_set_option_' . $option, $value );
 ...
 */
function movelogin_update_options( $options ) {
	// Make sure we don't mess everything.
	if ( ! is_array( $options ) ) {
		return;
	}

	update_site_option( MOVELOGIN_SETTINGS_SLUG, $options );
}
 ...
 * @return (mixed) The option value.
 */
function movelogin_get_module_option( $option, $default = null, $module = false ) {
	if ( ! $module ) {
		$module = movelogin_get_current_module();
	}
	 ...
	 */
	$value = apply_filters( 'pre_movelogin_get_module_option_' . $option, $pre, $default, $module );

	if ( null !== $value ) {
		return $value;
	}

	$options = get_site_option( "movelogin_{$module}_settings" );
	$value   = is_array( $options ) && isset( $options[ $option ] ) && false !== $options[ $option ] ? $options[ $option ] : $default;
	/**
	 * Filter any SecuPress option after read.
	 *
	 ...
	 * @param (string) $module  The module.
	*/
	return apply_filters( 'movelogin_get_module_option_' . $option, $value, $default, $module );
}
 ...
 * @return (array)
 */
function movelogin_get_scan_results() {
 ...
 * @return (array)
 */
function movelogin_get_fix_results() {
 ...
 */
function movelogin_delete_site_transient( $transient ) {
 ...
 */
function movelogin_get_site_transient( $transient ) {
 ...
 */
function movelogin_set_site_transient( $transient, $value ) {
 ...
 * @return (bool) true if successful, false otherwise.
 */
function movelogin_delete_transient( $transient ) {
 ...
 * @return (mixed) Value of transient.
 */
function movelogin_get_transient( $transient, $default = false ) {
 ...
 * @return bool False if value was not set and true if value was set.
 */
function movelogin_set_transient( $transient, $value ) {
 ...
 */
function movelogin_delete_module_option( $module ) {
	global $wpdb;
	delete_site_option( "movelogin_{$module}_settings" );
	delete_site_transient( 'movelogin_active_submodules' );
	$wpdb->query( $wpdb->prepare( 'DELETE FROM ' . $wpdb->options . ' WHERE option_name LIKE "movelogin_active_submodule_%" AND option_value = %s', $module ) );
}
 ...
 * @param (string) $module  The module slug (see array keys from `modules.php`). Default is the current module.
 */
function movelogin_update_module_option( $option, $value, $module = false ) {
	if ( ! $module ) {
		$module = movelogin_get_current_module();
	}
	...
	update_site_option( "movelogin_{$module}_settings", $options );
}
 ...
 * @param (string) $module The module slug (see array keys from `modules.php`). Default is the current module.
 */
function movelogin_update_module_options( $values, $module = false ) {
...
add_filter( 'pre_movelogin_get_module_option_' . 'advanced-settings_admin-bar', 'movelogin_shortcut_module_option_get_user_admin_bar_setting' );
/**
 * Make the old site setting "advanced-settings_admin-bar" a user setting now
 *
 * @since 2.3.19 movelogin- prefix
 * @since 2.3.18.1
 * @author Julio Potier
 * @param (string) $pre '1' shows the admin bar in WP menu
 * @return (string)
 **/
function movelogin_shortcut_module_option_get_user_admin_bar_setting( $pre ) {
	$user_id      = get_current_user_id();
	$user_setting = get_user_option( 'movelogin-advanced-settings_admin-bar', $user_id );
	if ( false !== $user_setting ) {
		$pre = $user_setting;
	} else {
		remove_filter( 'pre_movelogin_get_module_option_' . 'advanced-settings_admin-bar', 'movelogin_shortcut_module_option_get_user_admin_bar_setting' );
		$pre = movelogin_get_module_option( 'advanced-settings_admin-bar', '1' );
		update_user_option( $user_id, 'movelogin-advanced-settings_admin-bar', $pre, true );
	}
	return $pre;
}

add_filter( 'pre_movelogin_get_module_option_' . 'advanced-settings_expert-mode', 'movelogin_shortcut_module_option_get_user_expert_mode_setting' );
/**
 ...
 * @return (string)
 **/
function movelogin_shortcut_module_option_get_user_expert_mode_setting( $pre ) {
	$user_id      = get_current_user_id();
	$user_setting = get_user_option( 'movelogin-advanced-settings_expert-mode', $user_id );
	if ( false !== $user_setting ) {
		$pre = $user_setting;
	} else {
		remove_filter( 'pre_movelogin_get_module_option_' . 'advanced-settings_expert-mode', 'movelogin_shortcut_module_option_get_user_expert_mode_setting' );
		$pre = movelogin_get_module_option( 'advanced-settings_expert-mode', '1' );
		update_user_option( $user_id, 'movelogin-advanced-settings_expert-mode', $pre, true );
	}
	return $pre;
}

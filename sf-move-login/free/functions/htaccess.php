 ...
 * @return (bool)
 */
function movelogin_write_in_htaccess() {
	/**
	 ...
	 */
	return apply_filters( 'movelogin.write_in_htaccess', false );
}
 ...
 */
function movelogin_write_htaccess( $marker, $rules = false, $relative_path = '' ) {
	global $is_apache;
	if ( ! $is_apache ) {
		return false;
	}

	$filesystem    = movelogin_get_filesystem();
	$htaccess_path = trailingslashit( movelogin_get_home_path() . trim( $relative_path, '/' ) );
	...
	if ( wp_is_writable( $htaccess_file ) || ! $filesystem->exists( $htaccess_file ) && wp_is_writable( $htaccess_path ) ) {
		...
		return movelogin_put_contents( $htaccess_file, $rules, array( 'marker' => $marker ) );
	}
 ...
 */
function movelogin_get_htaccess_marker( $function ) {
	$_function = 'movelogin_get_htaccess_' . $function;
	...
	*/
	$marker = apply_filters( 'movelogin.htaccess.marker_' . $function, $marker );

 ...
 */
function movelogin_class_path( $prefix, $class_name_part = '' ) {
	$folders = array(
		'scan'              => 'scanners',
		'singleton'         => 'common',
		'logs'              => 'common',
	...
	return MOVELOGIN_CLASSES_PATH . $folder . '/class-movelogin-' . $prefix . $class_name_part . '.php';
}
 ...
 * @param (string) $class_name_part The classes name is built as follow: "SecuPress_{$prefix}_{$class_name_part}".
 */
function movelogin_require_class( $prefix, $class_name_part = '' ) {
	$path = movelogin_class_path( $prefix, $class_name_part );
 ...
 * @author Grégory Viguier
 */
function movelogin_require_class_async() {
	/* https://github.com/A5hleyRich/wp-background-processing v1.0 */
	movelogin_require_class( 'Admin', 'wp-async-request' );
	movelogin_require_class( 'Admin', 'wp-background-process' );
}
 ...
 */
function movelogin_get_scanners() {
	$tests = array(
		'users-login' => array(
			0 => 'Admin_User',
			1 => 'Easy_Login',
	...
	return apply_filters( 'movelogin.scanner.tests', $tests );
}
 ...
 */
function movelogin_get_tests_for_ms_scanner_fixes() {
	return array(
		'Bad_Old_Plugins',
		'Bad_Old_Themes',
		'Subscription',
 ...
 */
function movelogin_get_scanner_counts( $type = '' ) {
	static $counts;
	if ( ! isset( $counts ) ) {
		$tests_by_status = movelogin_get_scanners();
		$scanners        = movelogin_get_scan_results();
		$fixes           = movelogin_get_fix_results();
		...
		switch ( strlen( $label ) ) {
			case 3:
				$css_class = ' movelogin-grade-plus-plus';
			break;
			case 2:
				$css_class = ' movelogin-grade-plus';
			break;
			default:
				$css_class = '';
			break;
		...
		switch ( $counts['grade'] ) {
			case 'A':
				$counts['text']  = __( 'Congratulations! 🎉', 'movelogin' );
				$counts['color'] = '43,205,193';
				break;
			case 'B':
				$counts['text']  = __( 'Almost perfect!', 'movelogin' );
				$counts['color'] = '241,196,15';
				break;
			case 'C':
				$counts['text']  = __( 'Not bad, but try to fix more items.', 'movelogin' );
				$counts['color'] = '247,171,19';
				break;
			case 'D':
				$counts['text']  = __( 'Well, it’s not good yet.', 'movelogin' );
				$counts['color'] = '242,41,94';
				break;
			case 'E':
				$counts['text']  = __( 'Better than nothing, but still not good.', 'movelogin' );
				$counts['color'] = '203,35,79';
				break;
			case 'F':
				$counts['text'] = __( 'Not good at all, fix more issues.', 'movelogin' );
				break;
			case 'G':
				$counts['text'] = __( 'Bad, fix issues right away!', 'movelogin' );
				break;
			case 'H':
				$counts['text'] = __( 'Still very bad, start fixing things!', 'movelogin' );
				break;
			case 'I':
				$counts['text'] = __( 'Very bad. You should take some actions.', 'movelogin' );
				break;
			case 'J':
				$counts['text'] = __( 'Very very bad, please fix something!', 'movelogin' );
				break;
			case 'K':
				$counts['text'] = __( 'Very very, really very bad.', 'movelogin' );
				break;
			case '∅':
				$counts['text'] = __( 'Error when saving the scanner results.', 'movelogin' ); // Old (ᕗ‶⇀︹↼)ᕗ彡┻━┻
				break;
		}
		$counts['subtext'] = sprintf( _n( 'Your grade is %1$s with %2$d good scanned item.', 'Your grade is %1$s with %2$d good scanned items.', $counts['good'], 'movelogin' ), $counts['letter'], $counts['good'] );
	}
 ...
 * @return (bool)
 */
function movelogin_can_perform_extra_fix_action() {
	global $pagenow;
 ...
 * @return (bool)
 */
function movelogin_is_plugin_active( $plugin ) {
	$plugins = (array) get_option( 'active_plugins', [] );
	$plugins = array_flip( $plugins );
	return isset( $plugins[ $plugin ] ) || movelogin_is_plugin_active_for_network( $plugin );
}
 ...
 * @return (bool)
 */
function movelogin_is_plugin_active_for_network( $plugin ) {
	if ( ! is_multisite() ) {
		return false;
	}
 ...
 */
function movelogin_die( $message = '', $title = '', $args = array() ) {
	$has_p           = strpos( $message, '<p>' ) !== false;
	$message         = ( $has_p ? '' : '<p>' ) . $message . ( $has_p ? '' : '</p>' );
	$message         = '<h1>' . MOVELOGIN_PLUGIN_NAME . '</h1>' . $message;
	$url             = movelogin_get_current_url( 'raw' );
	$force_die       = ! empty( $args['force_die'] );
	$context         = ! empty( $args['context'] ) ? $args['context'] : '';
	$is_scan_request = movelogin_is_scan_request(); // Used to bypass the whitelist for scans.
	$attack_type     = isset( $args['attack_type'] ) ? $args['attack_type'] : false;
	if ( $attack_type ) { // We can die when it's not an attack.
		movelogin_log_attack( $attack_type );
	}
	 ...
	 */
	$message = apply_filters( 'movelogin.die.message', $message, $url, $args, $is_scan_request, $context );
	 ...
	 * @param (bool)   $is_scan_request Tell if the request comes from one of our scans.
	 */
	do_action( 'movelogin.before.die', $message, $url, $args, $is_scan_request, $context );
 ...
 */
function movelogin_block( $module, $args = array( 'code' => 403 ) ) {
	$ip = movelogin_get_ip();
	 ...
	 * @param (string) $module The related module.
	 */
	$block_id = apply_filters( 'movelogin_block_id', $module );
	...
	$hidden = '***… // ' . sprintf( __( 'Hidden by %s.', 'movelogin' ), MOVELOGIN_PLUGIN_NAME );
	...
	$_REQUEST = apply_filters( 'movelogin.block.remove_content_from._REQUEST', $_REQUEST );
	$_GET     = apply_filters( 'movelogin.block.remove_content_from._GET',     $_GET     );
	$_POST    = apply_filters( 'movelogin.block.remove_content_from._POST',    $_POST    );
	$_COOKIE  = apply_filters( 'movelogin.block.remove_content_from._COOKIE',  $_COOKIE  );
	$_FILES   = apply_filters( 'movelogin.block.remove_content_from._FILES',   $_FILES   );
	...
	$args['b64']['URL']  = movelogin_get_current_url( 'raw' );
	$args['b64']['SP']   = movelogin_has_pro() ? 'Pro v' . MOVELOGIN_PRO_VERSION : 'Free v' . MOVELOGIN_VERSION;
	$args['b64']['ID']   = $module;
	$args['b64']['data'] = $data;
	 ...
	 */
	do_action( 'movelogin.block.' . $module, $ip, $args, $block_id );
	...
	$content = '<h2>' . $title . '</h2>';
	if ( ! $args['content'] ) {
		$content .= '<p>' . __( 'You are not allowed to access the requested page.', 'movelogin' ) . '</p>';
	} else {
		$content .= '<p>' . $args['content'] . '</p>';
	}

	$content .= '<h3>' . __( 'Logged Details:', 'movelogin' ) . '</h3>';
	$content .= '<p>';
	$content .= sprintf( __( 'Your IP: %s', 'movelogin' ), $ip ) . '<br>';
	$content .= sprintf( __( 'Time: %s', 'movelogin' ), date_i18n( _x( 'F j, Y g:i a', 'date', 'movelogin' ) ) ) . '<br>';
	$content .= sprintf( __( 'Reason: %s', 'movelogin' ), $block_id ) . '<br>';
	$content .= sprintf( __( 'Support ID: %s', 'movelogin' ), '<textarea style="width:100%;height:27px;vertical-align:text-top">' . base64_encode( json_encode( $args['b64'] ) ) . '</textarea>' ) . '<br>';
	$content .= '</p>';

	movelogin_die( $content, $title, [ 'response' => $args['code'], 'force_die' => true, 'attack_type' => $args['attack_type'] ] );
}
 ...
 */
function movelogin_is_scan_request() {
	return ! empty( $_SERVER['HTTP_X_MOVELOGIN_ORIGIN'] );
}
 ...
 */
function movelogin_admin_url( $page, $module = '' ) {
	if ( 'get-pro' === $page ) {
		return trailingslashit( set_url_scheme( MOVELOGIN_WEB_MAIN, 'https' ) ) . _x( 'pricing', 'link to website (Only FR or EN!)', 'movelogin' ) . $module;
	}

	$module = $module ? '&module=' . $module : '';
	$page   = str_replace( '&', '_', $page );
	
	// Use options-general.php for modules page since it's in Settings menu.
	if ( 'modules' === $page ) {
		$url = 'options-general.php?page=movelogin_modules' . $module;
	} else {
		$url = 'admin.php?page=' . MOVELOGIN_PLUGIN_SLUG . '_' . $page . $module;
	}
 ...
 */
function movelogin_get_capability( $force_mono = false, $context = '' ) {
	 ...
	 */
	return apply_filters( 'movelogin.user_capability', $role, $context );
}
 ...
 * @return (string)
 */
function movelogin_user_agent( $user_agent ) {
	return sprintf( '%s;SecuPress|%s|%s|%s|%s;', $user_agent, esc_url( movelogin_get_main_url() ), MOVELOGIN_VERSION, $GLOBALS['wp_version'], phpversion() );
}
 ...
 * @return (string) The URL.
 */
function movelogin_get_main_url() {
	...
	$scheme   = movelogin_server_is_ssl() ? 'https' : 'http';
 ...
 * @return (bool)
 */
function movelogin_is_white_label() {
	return false;
}
 ...
 */
function movelogin_get_logo( $atts = [], $return = 'html' ) {
	$is_pro    = movelogin_is_pro() ? '-pro' : '';
	$base_url  = MOVELOGIN_ADMIN_IMAGES_URL . 'logo' . $is_pro;
	$base_path = MOVELOGIN_ADMIN_PATH . 'images/logo' . $is_pro;
	if ( movelogin_is_white_label() ) {
		/**
		...
		$base_url = apply_filters( 'movelogin.white_label.logo', $base_url, $atts, $return );
	}
	switch ( $return ) {
		case 'url':
			return $base_url . '.png';
	...
	$atts = array_merge( [
		'src'    => "{$base_url}.png",
		'srcset' => movelogin_is_white_label() ? '' : "{$base_url}2x.svg 1x, {$base_url}2x.svg 2x",
		'alt'    => '',
	], $atts );
 ...
 * @return (string) The HTML tag.
 */
function movelogin_get_logo_word( $atts = array() ) {
	return sprintf( '%s v%s', MOVELOGIN_PLUGIN_NAME, MOVELOGIN_VERSION );
}
 ...
 * @return (bool)
 */
function movelogin_users_can_register() {
	if ( ! is_multisite() ) {
 ...
 * @return (string)
 */
function movelogin_get_email( $from_header = false ) {
	 ...
	 * @param (string)
	 */
	$email = apply_filters( 'movelogin.get_email', 'noreply@' . $sitename );
	$email = apply_filters( 'wp_mail_from', $email );


	return $from_header ? 'from: ' . MOVELOGIN_PLUGIN_NAME . ' <' . $email . '>' : $email;
}
 ...
 */
function movelogin_send_mail( $to, $subject, $message, $headers = [], $attachments = array() ) {
	 ...
	 * @author Julio Potier
	 */
	$replacement = apply_filters( 'movelogin.mail.replacement', $replacement );
	$replaced    = [ movelogin_get_blogname(), get_option( 'admin_email' ), home_url(), wp_login_url() ];
	/**
	 ...
	 * @author Julio Potier
	 */
	$replaced    = apply_filters( 'movelogin.mail.replaced', $replaced );
	 ...
	 * @author Grégory Viguier
	 */
	$headers = apply_filters( 'movelogin.mail.headers', $headers );
	 ...
	 * @author Julio Potier
	 */
	do_action( 'movelogin.mail.before.wp_mail', [ $to, $subject, $message, $headers, $attachments ] );
 ...
 * @return (string)
 */
function movelogin_get_blogname() {
	static $blogname;
 ...
 * @return (string)
 */
function movelogin_get_current_url( $mode = 'base' ) {
	...
	$url  = 'http' . ( movelogin_server_is_ssl() ? 's' : '' ) . '://' . $host . str_replace( '//', '/', $url );
 ...
 */
function movelogin_cache_data( $key ) {
	static $data = array();
 ...
 * @return (int)
 */
function movelogin_get_main_blog_id() {
	static $blog_id;
 ...
 */
function movelogin_wp_version_is( $version ) {
	global $wp_version;
	static $is = array();
 ...
 */
function movelogin_wp_installing() {
 ...
 * @return (bool)
 **/
function movelogin_is_site_ssl() {
	static $is_site_ssl;
	...
	if ( is_multisite() ) {
		switch_to_blog( movelogin_get_main_blog_id() );
	 ...
	 */
	$is_site_ssl = apply_filters( 'movelogin.front.is_site_ssl', $is_site_ssl );
 ...
 * @return (bool)
 */
function movelogin_in_array_deep( $needle, $haystack ) {
	if ( $haystack ) {
		foreach ( $haystack as $item ) {
			if ( $item === $needle || ( is_array( $item ) && movelogin_in_array_deep( $needle, $item ) ) ) {
				return true;
			}
		}
	}
 ...
 * @return (array)
 */
function movelogin_array_merge_intersect( $values, $default ) {
 ...
 */
function movelogin_get_consumer_email() {
	return is_email( movelogin_get_option( 'consumer_email' ) );
}
 ...
 * @return (string)
 */
function movelogin_get_consumer_key() {
	return movelogin_get_option( 'consumer_key' );
}
 ...
 * @return (bool)
 */
function movelogin_has_pro() {
	return defined( 'MOVELOGIN_PRO_VERSION' );
}
 ...
 * @return (bool)
 */
function movelogin_has_pro_license() {
	static $has_pro;

	if ( ! isset( $has_pro ) ) {
		$has_pro = movelogin_get_consumer_key() && 1 === movelogin_get_option( 'site_is_pro' );
	}
 ...
 * @return (bool)
 */
function movelogin_is_pro() {
	return movelogin_has_pro() && movelogin_has_pro_license();
}
 ...
 */
function movelogin_feature_is_expert( $feature ) {
	$features = [
		// Field names.
 ...
 */
function movelogin_feature_is_pro( $feature ) {
	$features = [
		// Field names.
 ...
 */
function movelogin_is_affected_role( $module, $submodule, $user ) {
	$roles = movelogin_get_module_option( $submodule . '_affected_role', [], $module );

	if ( ! $roles ) {
		return -1;
	}

	if ( ! movelogin_is_user( $user ) ) {
		$user = movelogin_get_user_by( $user );
	}
	...
	return movelogin_is_user( $user ) && ! array_intersect( $roles, $user->roles );
}
 ...
 * @return (int)
 */
function movelogin_modify_userid_for_nonces( $uid = 0, $action = '' ) {
	return crc32( $uid . $action . movelogin_get_ip() );
}
 ...
 * @return (bool)
 */
function movelogin_is_user( $user ) {
 ...
 * @return (array)
 */
function movelogin_get_user_metas( $meta_key ) {
	global $wpdb;
	$res = movelogin_cache_data( __FUNCTION__ . $meta_key );
	if ( ! $res ) {
		...
		movelogin_cache_data( __FUNCTION__ . $meta_key, $res );
	}
	return $res;
}
 ...
 */
function movelogin_compress_data( $data ) {
 ...
 */
function movelogin_decompress_data( $data ) {
 ...
 * @author Grégory Viguier
 */
function movelogin_maybe_increase_memory_limit() {
 ...
 *                          Default 'error'.
 */
function movelogin_add_settings_error( $setting, $code, $message, $type = 'error' ) {
...
/**
 * Fetch settings errors registered by `add_settings_error()` and `movelogin_add_settings_error()`.
 ...
 */
function movelogin_get_settings_errors( $setting = '', $sanitize = false ) {
	global $wp_settings_errors;
 ...
 */
function movelogin_is_function_disabled( $function ) {
...
/**
 * Return an aray of translated expert modules with links, set by MOVELOGIN_EXPERT_MODULES_ON GLOBAL var
 *
 * @since 2.3.17
 * @author Julio Potier
 * 
 * @return (array)
 **/
function movelogin_get_expert_modules_on() {
	global $MOVELOGIN_EXPERT_MODULES_ON;
	if ( empty( $MOVELOGIN_EXPERT_MODULES_ON ) ) {
		return;
	}
	$values['bad_url_access']   = '"<a href="' . movelogin_admin_url( 'modules', 'sensitive-data#row-content-protect_bad-url-access' ) . '">' . __( 'Bad Url Access', 'movelogin' )    . '</a>"';
	$values['plugin_actions']   = '"<a href="' . movelogin_admin_url( 'modules', 'plugins-themes#row-plugins_actions' ) . '">'                . __( 'Plugin Actions', 'movelogin' )    . '</a>"';
	$values['lexicomatisation'] = '"<a href="' . movelogin_admin_url( 'modules', 'users-login#row-blacklist-logins_activated' ) . '">'        . __( 'Rename user names', 'movelogin' ) . '</a>"';

	return array_intersect_key( $values, $MOVELOGIN_EXPERT_MODULES_ON );
}

function movelogin_no_contextual_help() {
	_deprecated_function( __FUNCTION__, '2.3.19', 'movelogin_show_contextual_help' );
	return ! movelogin_show_contextual_help();
}

/**
 * Returns true if MOVELOGIN_MODE contains "expert" or setting is on
 *
 * @since 2.3.19 Revamp
 * @since 2.3.17 main
 ...
 * @return (bool|null)
 **/
function movelogin_is_expert_mode( $from_constant = false ) {
	if ( defined( 'MOVELOGIN_MODE' ) && ( false !== strpos( strtolower( MOVELOGIN_MODE ), 'expert' ) ) ) {
		$constant = true;
	}
	if ( $from_constant ) {
	...
	return movelogin_get_expert_modules_on() || movelogin_get_module_option( 'advanced-settings_expert-mode-main', false, 'welcome' );
}

/**
 * Returns true if user option is '1' or MOVELOGIN_MODE does not contains "help"
 *
 * @since 2.3.18.1 
 * @since 2.3.17 
 * @author Julio Potier
 ...
 * @return (bool|null)
 **/
function movelogin_show_contextual_help( $from_constant = false ) {
	if ( defined( 'MOVELOGIN_MODE' ) && ( false !== strpos( strtolower( MOVELOGIN_MODE ), 'help' ) ) ) {
		$constant = false;
	}
	if ( $from_constant ) {
		return isset( $constant ) ? $constant : null;
	...
	return movelogin_get_module_option( 'advanced-settings_expert-mode', true, 'welcome' );
}

/**
 * Returns true if user option is '1' or MOVELOGIN_MODE contains "adminbar"
 *
 * @since 2.3.19
 * @author Julio Potier
 * 
 * @return (bool|null)
 **/
function movelogin_show_adminbar( $from_constant = false ) {
	if ( defined( 'MOVELOGIN_MODE' ) && ( false !== strpos( strtolower( MOVELOGIN_MODE ), 'adminbar' ) ) ) {
		$constant = false;
	}
	if ( $from_constant ) {
		return isset( $constant ) ? $constant : null;
	...
	return movelogin_get_module_option( 'advanced-settings_admin-bar', true, 'welcome' );
}

/**
 * Returns true if user option is '1' or MOVELOGIN_MODE contains "grade"
 *
 * @since 2.3.19
 * @author Julio Potier
 * 
 * @return (bool|null)
 **/
function movelogin_show_grade_system( $from_constant = false ) {
	if ( defined( 'MOVELOGIN_MODE' ) && ( false !== strpos( strtolower( MOVELOGIN_MODE ), 'grade' ) ) ) {
		$constant = false;
	}
	if ( $from_constant ) {
		return isset( $constant ) ? $constant : null;
	...
	return movelogin_get_module_option( 'advanced-settings_grade-system', true, 'welcome' );
}
/**
 ...
 * @return (void)
 **/
function movelogin_set_recursive_chmod_rights( $path = ABSPATH ) {
	...
	* @param (array) $exts
	*/
	$exts = apply_filters( 'movelogin.chmod.file_types', $exts );
	foreach ( $dir as $item ) {
		if ( $item->isDot() ) {
			continue;
		}
		if ( $item->isDir() ) {
			...
			movelogin_set_recursive_chmod_rights( $item->getPathname() );
 ...
 **/
function movelogin_site_is_using_https( $type = 'both' ) {
 ...
 *
 * @since 2.2.6 movelogin_server_is_ssl() = 
 * @since 2.0
 * @author Julio Potier
 *
 ...
 **/
function movelogin_is_https_supported() {
	if ( ! movelogin_server_is_ssl() ) {
		$error = new WP_Error();
		$error->add( 'invalid_ssl', sprintf( __( 'SSL is not supported on HTTP PORT %s', 'movelogin' ), esc_html( $_SERVER['SERVER_PORT'] ) ) );
		set_transient( 'movelogin_is_https_supported', $error, 6 * HOUR_IN_SECONDS );
		return false;
	}
	if ( ! wp_http_supports( [ 'ssl' ] ) ) {
		$error = new WP_Error();
		$error->add( 'invalid_ssl', __( 'SSL is not supported on this website.', 'movelogin' ) );
		set_transient( 'movelogin_is_https_supported', $error, 6 * HOUR_IN_SECONDS );
		return false;
	}
	$response = get_transient( 'movelogin_is_https_supported' );
	...
	set_transient( 'movelogin_is_https_supported', $response, 6 * HOUR_IN_SECONDS );
 ...
 * @return (bool)
 **/
function movelogin_server_is_ssl( $with_cloudflare = true ) {
	$is_ssl = false;
	 ...
	 */
	return apply_filters( 'movelogin.is_sll', $is_ssl, $with_cloudflare );
}
 ...
 **/
function movelogin_update_urls_to_https() {
...
/**
 * Before each usage of movelogin_send_slack_notification(), maybe check if this is still correct
 *
 * @since 2.0
 * @author Julio Potier
 *
 * @see movelogin_send_slack_notification()
 *
 * @return (bool) True if still OK, false if not.
 **/
function movelogin_maybe_reset_slack_notifs() {
	$url      = movelogin_get_module_option( 'notification-types_slack', false, 'alerts' );
	$accepted = movelogin_get_option( 'notification-types_slack', false );
	if ( apply_filters( 'movelogin.notifications.slack.bypass', false ) || ( ! empty( $accepted ) && $url === $accepted ) ) {
		return true;
	}
	movelogin_set_option( 'notification-types_slack', 0 );
	return false;
}
 ...
 * @return (bool)
 **/
function movelogin_remove_old_plugin_file( $file ) {
	// Is it a sym link ?
	if ( is_link( $file ) ) {
		$file = @readlink( $file );
	}
 ...
 * @return (string)
 **/
function movelogin_translate_user_role( $role ) {
	global $wp_roles;
	_x( 'Administrator', 'User role', 'movelogin' );
	_x( 'Editor',        'User role', 'movelogin' );
	_x( 'Author',        'User role', 'movelogin' );
	_x( 'Contributor',   'User role', 'movelogin' );
	_x( 'Subscriber',    'User role', 'movelogin' );
	$role        = isset( $wp_roles->role_names[ $role ] ) ? $wp_roles->role_names[ $role ] : $role;
	$translation = translate_user_role( $role, 'movelogin' );
 ...
 **/
function movelogin_time_limit( $seconds ) {
 ...
 */
function movelogin_status( $status ) {
	$statuses            = [];
	$statuses['bad']     = _x( 'Bad', 'scan result', 'movelogin' );
	$statuses['good']    = _x( 'Good', 'scan result', 'movelogin' );
	$statuses['warning'] = _x( 'Pending', 'scan result', 'movelogin' );
	$statuses['cantfix'] = _x( 'Error', 'scan result', 'movelogin' );

	return isset( $statuses[ $status ] ) ? $statuses[ $status ] : _x( 'New', 'scanner item', 'movelogin' );
}
 ...
 */
function movelogin_format_message( $msgs, $test_name ) {
 ...
 * 
 **/
function movelogin_log_attack( $type ) {
}
 ...
 **/
function movelogin_is_mobile( $ua = null ) {
	if ( is_null( $ua ) ) {
		return wp_is_mobile();
	}
	$is_mobile = false;
 ...
 **/
function movelogin_plugins_api( $plugin_slug ) {

	$api = get_site_transient( 'movelogin_plugins_api_' . $plugin_slug );
	if ( false === $api ) {
		...
		if ( ! is_object( $api ) || is_wp_error( $api ) ) {
			return new WP_Error( 'plugin_not_found', __( 'Plugin not found', 'movelogin' ) );
		}
		$api->source = 'wp_org';
		set_site_transient( 'movelogin_plugins_api_' . $plugin_slug, $api, WEEK_IN_SECONDS );
	} else {
		$api->source = 'db';
	}
 ...
 * @return (string)
 */
function movelogin_get_server_type() {
 ...
 * @return (string)
 */
function movelogin_get_function_name_by_server_type( $prefix, $default = '__return_empty_string' ) {
	global $is_apache, $is_nginx, $is_iis7;
	
	$server_type = movelogin_get_server_type();
 ...
 **/
function movelogin_is_soft_request() {
	global $pagenow;
 ...
 */
function movelogin_get_user_data_by( $field, $value, $like = false, $sanitize_cb = '' ) {
	global $wpdb;
	...
	if ( $like ) {
		$value = movelogin_esc_like( $value );
	}
 ...
 *
 * @since 2.3.18.1 Revamp to use movelogin_get_user_data_by()
 * @since 2.2.6
 * @author Julio Potier
 *
 * @see get_user_by()
 * @see movelogin_get_user_data_by()
 * 
 * @param (string|int) $value The field value.
 * 
 * @return (bool|WP_User) False or WP_User
 */
function movelogin_get_user_by( $value, $return_field = '' ) {
	// Already a user.
	if ( movelogin_is_user( $value ) ) {
		return $value;
	}
	$user = false;
	$by   = '';
	...
	if ( is_int( $value ) ) {
		$by = 'ID';
		$user = movelogin_get_user_data_by( $by, $value );
	}
	...
	if ( ! movelogin_is_user( $user ) && is_email( $value ) ) {
		$by   = 'login';
		$user = movelogin_get_user_data_by( $by, $value );
	}
	...
	if ( $return_field ) {
		if ( isset( $user->{$return_field} ) ) {
			$user->movelogin_get_user_by_field = $return_field;
	...
	$user->movelogin_get_user_by_field = $by;

	return $user;
}

add_filter( 'wp_login_errors', 'movelogin_display_relogin_message' );
/**
 ...
 */
function movelogin_display_relogin_message( $errors ) {
	if ( ! isset( $_GET['movelogin-relog'] ) || empty( wp_get_referer() ) || ! movelogin_is_soft_request() ) {
		return $errors;
	}
	...
	switch ( $_GET['movelogin-relog'] ) {
		...
		case 'new-login':
			$errors->add( 'movelogin_relog', __( 'You will receive your new login confirmation in your mailbox.', 'movelogin' ), 'message' );
		break;
		
		case 'new-password':
			$errors->add( 'movelogin_relog', __( 'You will receive your new password confirmation in your mailbox.', 'movelogin' ), 'message' );
		break;
						
		default: // Should be an integer, user_id.
			$user_id = (int) $_GET['movelogin-relog'];
			if ( $user_id ) { // user_id 0 exists for cron jobs, we don't want them.
				$notices = movelogin_get_transient( 'movelogin-notices-' . $user_id, [] );
				if ( is_array( $notices ) && ! empty( $notices ) ) {
					delete_transient( 'movelogin-notices-' . $user_id );

...
defined( 'MOVELOGIN_VERSION' ) or die( 'Something went wrong.' );
 ...
 * @return (array)
 */
function movelogin_move_login_get_default_slugs() {
	$slugs = array(
		// custom.
		'login'                  => 1,
		// hardcoded.
 ...
 * @return (array)
 */
function movelogin_move_login_get_slugs() {
	$slugs = movelogin_move_login_get_default_slugs();
	$login_slug = movelogin_get_module_option( 'move-login_slug-login', 'login', 'users-login' );
	$login_slug = sanitize_title( $login_slug, 'login', 'display' );

	foreach ( $slugs as $action => $dummy ) {
		$slugs[ $action ] = movelogin_get_module_option( 'move-login_slug-' . $action, $action, 'users-login' );
	...
	// Those ones needs to be related to the login one, so we can't change them.
	$slugs['confirm_admin_email']     = $login_slug . '-confirm_admin_email';
	$slugs['lostpassword']            = $login_slug . '-lostpassword';
	$slugs['logout']                  = $login_slug . '-logout';
	$slugs['register']                = $login_slug . '-register';
	$slugs['resetpass']               = $login_slug . '-resetpass';
	$slugs['registration_disabled']   = $login_slug . '-registration_disabled';
...
add_filter( 'site_url', 'movelogin_move_login_site_url', 10, 4 );
/**
 * Filter the site URL.
 *
 * @since 1.0
 ...
 */
function movelogin_move_login_site_url( $url, $path, $scheme, $blog_id = null ) {
	if ( movelogin_is_submodule_active( 'users-login', 'move-login' ) && ! empty( $path ) && is_string( $path ) && false === strpos( $path, '..' ) && 0 === strpos( ltrim( $path, '/' ), 'wp-login.php' ) ) {
		$blog_id = (int) $blog_id;
		// Base url.
		...
		$url = rtrim( $url, '/' ) . movelogin_move_login_set_path( $path );
	}
	return $url;
}


add_filter( 'network_site_url', 'movelogin_move_login_network_site_url', 10, 3 );
/**
 ...
 */
function movelogin_move_login_network_site_url( $url, $path, $scheme ) {
	if ( movelogin_is_submodule_active( 'users-login', 'move-login' ) && ! empty( $path ) && is_string( $path ) && false === strpos( $path, '..' ) && 0 === strpos( ltrim( $path, '/' ), 'wp-login.php' ) ) {
...
add_filter( 'logout_url', 'movelogin_move_login_logout_url', 1 );
/**
 ...
 */
function movelogin_move_login_logout_url( $logout_url ) {
	return movelogin_move_login_login_to_action( $logout_url, 'logout' );
}


add_filter( 'lostpassword_url', 'movelogin_move_login_lostpassword_url', 1 );
/**
 ...
 */
function movelogin_move_login_lostpassword_url( $lostpassword_url ) {
	return movelogin_move_login_login_to_action( $lostpassword_url, 'lostpassword' );
}


add_filter( 'wp_redirect', 'movelogin_move_login_redirect', 10 );
/**
 ...
 */
function movelogin_move_login_redirect( $location ) {
	if ( ! movelogin_is_submodule_active( 'users-login', 'move-login' ) ) {
		return $location;
	}
	...
	if ( site_url( $location_base ) === site_url( 'wp-login.php' ) && strpos( wp_get_referer(), 'wp-login.php' ) === false ) {
		return movelogin_move_login_site_url( $location, $location, 'login', get_current_blog_id() );
	}
	...
	if ( trailingslashit( $location_base ) === trailingslashit( wp_login_url() ) ) {
		...
		return movelogin_move_login_login_to_action( $location, $location_query['action'] );
	}
...
add_filter( 'update_welcome_email', 'movelogin_move_login_update_welcome_email', 10, 2 );
/**
 ...
 */
function movelogin_move_login_update_welcome_email( $welcome_email, $blog_id ) {
 ...
 */
function movelogin_move_login_set_path( $path ) {
	$slugs = movelogin_move_login_get_slugs();
 ...
 */
function movelogin_move_login_login_to_action( $link, $action ) {
...
add_action( 'login_head', 'movelogin_hack_global_error' );
/**
 ...
 * @author Julio Potier
 **/
function movelogin_hack_global_error() {
	global $error;
	if ( '404' === $error ) {
...
add_filter( 'user_request_action_email_content', 'movelogin_user_request_action_email_content_move_login_url', 1, 2 );
/**
 ...
 * @author Julio Potier
 **/
function movelogin_user_request_action_email_content_move_login_url( $email_text, $email_data ) {
...
add_filter( 'rewrite_rules_array', 'movelogin_move_login_remove_rewrite_rules' );
/**
 ...
 * @return (array)
 */
function movelogin_move_login_remove_rewrite_rules( $rules ) {
	if ( ! is_multisite() ) {
...
add_action( 'login_init', 'movelogin_move_login_maybe_deny_login_page', 0 );
add_action( 'secure_auth_redirect', 'movelogin_move_login_maybe_deny_login_page', 0 );
/**
 ...
 * @author Grégory Viguier
 */
function movelogin_move_login_maybe_deny_login_page( $secure = true ) {
	/**
	 ...
	 */
	$bypass = apply_filters( 'movelogin.plugins.move_login.deny_bypass', is_user_logged_in() );
	if ( $bypass ) {
		return $secure;
	}
	...
	$parsed = trim( $parsed, '/' );
	$subdir = movelogin_get_wp_directory();
	$slugs  = movelogin_move_login_get_slugs();
	if ( $subdir ) {
	...
	// You shall not pass!
	movelogin_move_login_deny_login_access();
}
 ...
 * @author Grégory Viguier
 */
function movelogin_move_login_deny_login_access() {
	if ( has_action( 'movelogin.plugin.move-login.deny_login_access' ) ) {
		_deprecated_hook( 'movelogin.plugin.move-login.deny_login_access', '2.2.6', 'movelogin.plugins.move_login.deny_login_access' );
	}
	do_action( 'movelogin.plugins.move-login.deny_login_access' );
	/**
	 ...
	 * @author Grégory Viguier
	 */
	do_action( 'movelogin.plugins.move_login.deny_login_access' );

	movelogin_die( __( 'You are not allowed to access this page.', 'movelogin' ), '', [ 'force_die' => true, 'attack_type' => 'move_login' ]
	);
}


add_action( 'wp', 'movelogin_fallback_slug_redirect', 0 );
/**
 ...
 * @author Julio Potier
 **/
function movelogin_fallback_slug_redirect( $wp, $test = false ) {
	if ( ! $test && ( ! is_404() || ! isset( $_SERVER['REQUEST_URI'] ) ) ) {
		return;
	}
	$slugs  = movelogin_move_login_get_slugs();
	$base   = movelogin_get_wp_directory();
	$regex  = '^' . $base . '(' . implode( '|', $slugs ) . ')$';
	$parsed = wp_parse_url( $_SERVER['REQUEST_URI'] );
	$parsed = ! empty( $parsed['path'] ) ? $parsed['path'] : '';
	$parsed = trim( $parsed, '/' );
	if ( preg_match( "@{$regex}@", $parsed ) ) {
		$slugs  = array_flip( movelogin_move_login_get_slugs() );
...
add_action( 'setup_theme', 'movelogin_set_wp_lang_early', 1 );
/**
 ...
 * @author Julio Potier
 *
 * @see movelogin_set_wp_lang()
 *
 **/
function movelogin_set_wp_lang_early( $locale ) {
	if ( movelogin_fallback_slug_redirect( null, true ) ) {
		add_filter( 'determine_locale', 'movelogin_set_wp_lang' );
	}
}
 ...
 **/
function movelogin_set_wp_lang( $locale ) {
...
add_filter( 'register_url', 'movelogin_register_url_redirect' );
/**
 ...
 * @return (string) $url
 **/
function movelogin_register_url_redirect( $url ) {
	if ( ! is_user_logged_in() ) {
		$current_url = movelogin_get_current_url( 'raw' );
		if ( $url === $current_url ) {
			movelogin_move_login_deny_login_access();
		}
	}
...
add_filter( 'wp_login_errors', 'movelogin_register_errors' );
/**
 ...
 **/
function movelogin_register_errors( $errors ) {
	if ( isset( $_REQUEST['action'] ) && 'registration_disabled' === $_REQUEST['action'] ) {
		if ( isset( $_REQUEST['reauth'] ) ) {
			wp_clear_auth_cookie();
		}
		login_header( __( 'Log In', 'movelogin' ), '', $errors );
		login_footer();
		die();
	}
	return $errors;

/* globals jQuery: false, ajaxurl: false, wp: false, MoveLogini18nModules: false, secupressIsSpaceOrEnterKey: false, swal2: false */
// Global vars =====================================================================================
var SecuPress = {
	doingAjax:           {},
	...
	swal2Defaults:        {
		title:             MoveLogini18nModules.confirmTitle,
		confirmButtonText: MoveLogini18nModules.confirmText,
		cancelButtonText:  MoveLogini18nModules.cancelText,
		type:              "warning",
	...
	expandButton: '<p class="secupress-expand-trigger-container"><button class="secupress-expand-trigger" type="button"><span class="secupress-expand-txt">' + MoveLogini18nModules.expandTextOpen + '</span><i class="secupress-icon-angle-down" aria-hidden="true"></i></button></p>'
};

/**
 * Show swal2 message if no scan done yet
 */
if ( MoveLogini18nModules.alreadyScanned === 0 ) {
	var modal_title = '<span class="secupress-swal-header-imaged"><img src="' + MoveLogini18nModules.firstScanImage + '" alt="" width="113" height="113"></span>',
		modal_content;

	modal_content  = '<p class="secupress-text-baseup secupress-mt1 secupress-mb1 secupress-primary secupress-bold">';
		modal_content += MoveLogini18nModules.firstScanTitle;
	modal_content += '</p>';
	modal_content += '<p class="secupress-text-base secupress-mt2 secupress-mb2">';
		modal_content += MoveLogini18nModules.firstScanText;
	modal_content += '</p>';
	modal_content += '<p class="secupress-mt1 secupress-mb1">';
		modal_content += '<a class="secupress-button secupress-button-primary secupress-button-scan shadow" href="' + MoveLogini18nModules.firstScanURL + '">';
			modal_content += '<span class="icon">';
				modal_content += '<i class="secupress-icon-radar" aria-hidden="true"></i>';
			modal_content += '</span>';
			modal_content += '<span class="text">';
				modal_content += MoveLogini18nModules.firstScanButton;
			modal_content += '</span>';
		modal_content += '</a>';
	modal_content += '</p>';
...
function secupressDisplayAjaxError( $button, text, ajaxID ) {
	if ( undefined === text ) {
		text = MoveLogini18nModules.unknownError;
	}

	swal2( jQuery.extend( {}, SecuPress.swal2Defaults, {
		title: MoveLogini18nModules.error,
		html:  text,
		type:  "error"
	} ) );
...
function secupressDisplayAjaxSuccess( $button, text, ajaxID ) {
	...
	swal2( jQuery.extend( {}, SecuPress.swal2Defaults, {
		title: MoveLogini18nModules.done,
		html:  text,
		type:  "success",
		timer: 4000
	} ) );
	...
	if ( "function" === typeof document.createElement( "input" ).checkValidity ) {
		$( ".affected-role-row :checkbox" ).on( "click.secupress", function() {
			...
			if ( 0 === $( '[name="' + this.name + '"]:checked' ).length ) {
				this.setCustomValidity( MoveLogini18nModules.selectOneRoleMinimum );
...
(function($, d, w, undefined) {
	...
	if ( MoveLogini18nModules.moveLoginNonce ) {
			...
			timeout[ elem.id ] = w.setTimeout( function() {
				var $elem  = $( elem ).addClass( 'ui-autocomplete-loading' ),
					params = {
						...
						'slug':     value,
						'default':  action,
						'_wpnonce': MoveLogini18nModules.moveLoginNonce
					};
				...
				.done( function( r ) {
					if ( $.isPlainObject( r ) && r.success ) {
						...
						secupressUpdateMoveLoginSlug( '--' + MoveLogini18nModules.error + '--', $elem );
					}
				} )
				.fail( function() {
					secupressUpdateMoveLoginSlug( '--' + MoveLogini18nModules.error + '--', $elem );
...
(function($, d, w, undefined) {
	...
	function secupressDeleteAllBackups( $button, href ) {
		secupressDisableAjaxButton( $button, MoveLogini18nModules.deletingAllText, "backup" );
		...
		.done( function( r ) {
			...
			if ( $.isPlainObject( r ) && r.success ) {
				...
				secupressEnableAjaxButton( $button, MoveLogini18nModules.deletedAllText, "backup" );
			} else {
				secupressDisplayAjaxError( $button, MoveLogini18nModules.deleteAllImpossible, "backup" );
			}
		} )
		.fail( function() {
	...
	function secupressDeleteOneBackup( $button, href ) {
		secupressDisableAjaxButton( $button, MoveLogini18nModules.deletingOneText, "backup" );
		...
		.done( function( r ) {
			if ( $.isPlainObject( r ) && r.success ) {
				...
				if ( wp.a11y && wp.a11y.speak ) {
					wp.a11y.speak( MoveLogini18nModules.deletedOneText );
				}
			} else {
				secupressDisplayAjaxError( $button, MoveLogini18nModules.deleteOneImpossible, "backup" );
			}
		} )
		.fail( function() {
			secupressDisplayAjaxError( $button, null, "backup" );
	...
	function secupressDoDbBackup( $button, href ) {
		secupressDisableAjaxButton( $button, MoveLogini18nModules.backupingText, 'backup' );
		...
		.done( function( r ) {
			if ( $.isPlainObject( r ) ) {
				if ( r.success ) {
					...
					secupressUpdateAvailableBackupCounter( r );
					secupressUpdateBackupVisibility();
					secupressEnableAjaxButton( $button, MoveLogini18nModules.backupedText, "backup" );
				} else {
					r.data = r.data ? r.data : MoveLogini18nModules.backupImpossible;
					secupressDisplayAjaxError( $button, r.data, "backup" );
				}
			} else {
				secupressDisplayAjaxError( $button, MoveLogini18nModules.backupImpossible, "backup" );
			}
		} )
		.fail( function() {
			secupressDisplayAjaxError( $button, null, "backup" );
	...
	$( "#submit-delete-backups" ).on( "click.secupress", function( e ) {
		...
		if ( "function" === typeof w.swal2 ) {
			swal2( $.extend( {}, SecuPress.swal2Defaults, SecuPress.swal2ConfirmDefaults, {
				text:              MoveLogini18nModules.confirmDeleteBackups,
				confirmButtonText: MoveLogini18nModules.yesDeleteAll,
				type:              "question",
				reverseButtons:    true
		...
		} else if ( w.confirm( MoveLogini18nModules.confirmTitle + "\n" + MoveLogini18nModules.confirmDeleteBackups ) ) {
			secupressDeleteAllBackups( $this, href );
		}
	...
	$( "body" ).on( "click.secupress keyup", ".a-delete-backup", function( e ) {
		...
		if ( "function" === typeof w.swal2 ) {
			swal2( $.extend( {}, SecuPress.swal2Defaults, SecuPress.swal2ConfirmDefaults, {
				text:              MoveLogini18nModules.confirmDeleteBackup,
				confirmButtonText: MoveLogini18nModules.yesDeleteOne,
				type:              "question",
				reverseButtons:    true
			} ) ).then( function ( isConfirm ) {
				if ( isConfirm ) {
		...
		} else if ( w.confirm( MoveLogini18nModules.confirmTitle + "\n" + MoveLogini18nModules.confirmDeleteBackup ) ) {
			secupressDeleteOneBackup( $this, href );
		}
	} );
...
(function($, d, w, undefined) {
	...
	function secupressBannedIPsEmptyList( message, resetSearch ) {
		...
		if ( undefined === message || ! message ) {
			message = MoveLogini18nModules.noBannedIPs;
		}
	...
	function secupressBannedIPsFillList( data, replace ) {
		var $list    = $row.find( "#secupress-banned-ips-list" ),
			template = '<li class="secupress-large-row" data-ip="%ip%"><strong>%ip%</strong> <em>(%time%)</em><span><a class="a-unban-ip" href="%unban_url%">' + MoveLogini18nModules.delete + '</a> <span class="spinner secupress-inline-spinner"></span></span></li>',
		...
		if ( undefined === data || ! data.length ) {
			...
			if ( replace ) {
				...
				if ( typeof replace === "string" ) {
					...
					secupressBannedIPsEmptyList( MoveLogini18nModules.IPnotFound, false );
				} else {
	...
	function secupressBanIP( $button, href ) {
		...
		.done( function( r ) {
			...
			if ( ! $.isPlainObject( r ) || ! r.data || ! $.isPlainObject( r.data ) ) {
				secupressDisplayAjaxError( $button, MoveLogini18nModules.error, "ban-ip" );
				return;
			}
	...
	$row.on( "click.secupress keyup", "#reset-banned-ips-list", function( e ) {
		...
		$row.load( href + " #banned-ips-row > th, #banned-ips-row > td", function() {
			...
			secupressEnableAjaxButton( $this, MoveLogini18nModules.searchReset, "ban-ip" );
		} );
	} );
	...
	$row.on( "click.secupress keyup", ".a-unban-ip", function( e ) {
		...
		.done( function( r ) {
			...
			if ( ! $.isPlainObject( r ) || ! r.data || ! $.isPlainObject( r.data ) ) {
				secupressDisplayAjaxError( $this, MoveLogini18nModules.error, "ban-ip" );
				return;
			}
			...
			if ( ! $list.children().length ) {
				if ( $( "#reset-banned-ips-list" ).hasClass( "hidden" ) ) {
					...
					// It's a search.
					secupressBannedIPsEmptyList( MoveLogini18nModules.IPremoved, false );
				}
			}
...
(function($, d, w, undefined) {
	...
	function secupressWhitelistIPsEmptyList( message, resetSearch ) {
		...
		if ( undefined === message || ! message ) {
			message = MoveLogini18nModules.noWhitelistIPs;
		}
		// Remove all rows from the list and display the placeholder.
	...
	function secupressWhitelistIPsFillList( data, replace ) {
		var $list    = $row.find( "#secupress-whitelist-ips-list" ),
			template = '<li class="secupress-large-row" data-ip="%ip%"><strong>%ip%</strong> <span><a class="a-unwhitelist-ip" href="%unwhitelist_url%">' + MoveLogini18nModules.delete + '</a> <span class="spinner secupress-inline-spinner"></span></span></li>',
		...
		if ( undefined === data || ! data.length ) {
			...
			if ( replace ) {
				...
				if ( typeof replace === "string" ) {
					...
					secupressWhitelistIPsEmptyList( MoveLogini18nModules.IPnotFound, false );
				} else {
	...
	function secupressWhitelistIP( $button, href ) {
		...
		.done( function( r ) {
			...
			if ( ! $.isPlainObject( r ) || ! r.data || ! $.isPlainObject( r.data ) ) {
				secupressDisplayAjaxError( $button, MoveLogini18nModules.error, "whitelist-ip" );
				return;
			}
	...
	$row.on( "click.secupress keyup", "#reset-whitelist-ips-list", function( e ) {
		...
		$row.load( href + " #whitelist-ips-row > th, #whitelist-ips-row > td", function() {
			...
			secupressEnableAjaxButton( $this, MoveLogini18nModules.searchReset, "whitelist-ip" );
		} );
	} );
	...
	$row.on( "click.secupress keyup", ".a-unwhitelist-ip", function( e ) {
		...
		.done( function( r ) {
			...
			if ( ! $.isPlainObject( r ) || ! r.data || ! $.isPlainObject( r.data ) ) {
				secupressDisplayAjaxError( $this, MoveLogini18nModules.error, "whitelist-ip" );
				return;
			}
			...
			if ( ! $list.children().length ) {
				if ( $( "#reset-whitelist-ips-list" ).hasClass( "hidden" ) ) {
					...
					// It's a search.
					secupressWhitelistIPsEmptyList( MoveLogini18nModules.IPremoved, false );
				}
			}
...
(function($) {
	...
	$expandables.closest( 'label, .secupress-setting-content-col' ).on( 'click.secupress', '.secupress-expand-trigger', function() {
		...
		if ( $_this.hasClass( 'open' ) ) {
			...
			$_this.find( '.secupress-expand-txt' ).text( MoveLogini18nModules.expandTextOpen );
			...
			$textarea.focusToEnd();
			$_this.addClass( 'open' );
			$_this.find( '.secupress-expand-txt' ).text( MoveLogini18nModules.expandTextClose );
	...
	$( 'fieldset.secupress-boxed-group' ).each( function() {
		...
		$box.next( '.secupress-expand-trigger-container' ).on( 'click.secupress', function() {
			...
			if ( $this.hasClass( 'open' ) ) {
				...
				$this.find( '.secupress-expand-txt' ).text( MoveLogini18nModules.expandTextOpen );
				...
				$this.addClass( 'open' );
				$this.find( '.secupress-expand-txt' ).text( MoveLogini18nModules.expandTextClose );
...
// Malware Scan Status =============================================================================
(function($, d, w, undefined) {
	if ( undefined !== MoveLogini18nModules && 'on' === MoveLogini18nModules.malwareScanStatus ) {
		window.stop();
		...
		function secupress_get_malwarescanstatus() {
			...
			$.getJSON( ajaxurl, params ).done( function( r ) {
				if ( ! r.success ) {
					$("#secupress-scanner-info code").parent().parent().text( MoveLogini18nModules.malwareScanError );
				} else if( r.data.malwareScanStatus ) {
					w.location.href = MoveLogini18nModules.MalwareScanURI;
				} else {
...
(function( w, d, $, undefined ) {
	$( '#button-update-malware-data' ).on( 'click', function(e) {
		...
		.done( function( r ) {
			if ( r.success ) {
				if ( wp.a11y && wp.a11y.speak ) {
					wp.a11y.speak( MoveLogini18nModules.malwareUpdateOK );
				}
				$('p.description.warning.secupress-show-expert').text( MoveLogini18nModules.malwareUpdateOK ).removeClass('warning').addClass('help');
				try {
					location.reload();
				} catch (e) {
				...
				secupressDisplayAjaxError( _this, MoveLogini18nModules.malwareUpdateKO, "data" );
			}
		} )
		.fail( function() {
...
(function($, d, w, undefined) {
	$( '.secupressicon-reset' ).on( 'click', function(e) {
		...
		e.preventDefault();
		swal2( $.extend( {}, SecuPress.swal2Defaults, SecuPress.swal2ConfirmDefaults, {
			text:              MoveLogini18nModules.resetDefault,
			type:              "question",
			reverseButtons:    true
		} ) ).then( function ( isConfirm ) {
			if ( isConfirm ) {
...
(function($, d, w, undefined) {
	$( '#secupress-regen-keys' ).on( 'click', function(e) {
		...
		e.preventDefault();
		swal2( $.extend( {}, SecuPress.swal2Defaults, SecuPress.swal2ConfirmDefaults, {
			text:              MoveLogini18nModules.regenKeys,
			type:              "question",
			reverseButtons:    true
		} ) ).then( function ( isConfirm ) {
			if ( isConfirm ) {
